...
#endif
{
    ...
    simdProcessor = std::make_unique<SIMDProcessor>(treeState);
}
...
void PluginAudioProcessor::prepareToPlay (double sampleRate, int samplesPerBlock)
{
    ...
    specs.maximumBlockSize = juce::uint32(samplesPerBlock);
    specs.numChannels = 2;
    
    simdProcessor->prepare(specs);
}
...
void PluginAudioProcessor::processBlock (juce::AudioBuffer<float>& buffer, juce::MidiBuffer& midiMessages)
{
    ...
    juce::dsp::AudioBlock<float> audioBlock(buffer.getArrayOfWritePointers(), numChannels,numSamples);
    juce::dsp::ProcessContextReplacing<float> context(audioBlock);
    
    simdProcessor->process(context);
